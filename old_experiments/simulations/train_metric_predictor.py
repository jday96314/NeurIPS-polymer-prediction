...
if __name__ == '__main__':
    # LOAD DATA.
    # raw_train_df = pl.read_csv('simulations/generated_metrics/PI1M_hybrid_09_07.csv')
    raw_train_df = pl.read_csv('simulations/generated_metrics/PI1M_hybrid_09_13.csv')
    raw_test_df = pl.read_csv('simulations/generated_metrics/host_hybrid.csv')

    # COMPUTE FEATURES
    FEATURES_CONFIG = { # Baseline 0.57954 --> 0.6019 (with tweaks)
        # 'morgan_fingerprint_dim': 1024,
        'morgan_fingerprint_dim': 2048,
        # 'atom_pair_fingerprint_dim': 512,
    ...
    for config_path in tqdm(config_paths):
        ...
        joblib.dump(model, f'simulations/models_v2/xgb_{target_name}.joblib')
